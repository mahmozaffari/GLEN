...
@registry.register_trainer("ensemble_logits")
class EnsembleTrainerLogits(
    TrainerCallbackHookMixin,
    TrainerTrainingLoopMixin,
    TrainerDeviceMixin,
    TrainerEnsembleEvaluationLoopMixin,
    ...
    def load(self):
        ...
        self.read_model_logits(self.config.ensemble.params)
    ...
    def read_model_logits(self, logits_dirs):
        logger.info("Ensemble models:")
        logger.info(logits_dirs)
        # logits_dirs: contain a list of logit outputs from sub-networks to be ensembled
        # returns combined model logits
        model_logits = dict()
        count = dict()
        for logit_dir in  logits_dirs:
            logger.info('===================== Loading logits from {} ====================='.format(logit_dir))
            if os.path.exists(os.path.join(logit_dir,'logits.pth')):
                logger.info('loading logits.pth')
                temp_logits = torch.load(os.path.join(logit_dir,'logits.pth'))
                logger.info('logits.pth loaded')
                for qi in temp_logits:
                    if qi not in model_logits:
                        model_logits[qi] = temp_logits[qi]
                        count[qi] = 1
                    else:
                        model_logits[qi] += temp_logits[qi]
                        count[qi] += 1
            else:
                for f in glob.glob(os.path.join(logit_dir,'logits','*.pth')):
                    qi = int(f.split('/')[-1].split('.')[0])
                    logit = torch.load(f)
                    if qi not in model_logits:
                        model_logits[qi] = logit
                        count[qi] = 1
                    else:
                        model_logits[qi] += logit
                        count[qi] += 1

        for qi in model_logits:
            assert(count[qi] == len(logits_dirs))
            model_logits[qi] /= count[qi]
            
        self.ensemble_logits = model_logits
    ...
    def train(self):
        ...
        logger.info("====== Ensemble Models ======")
        for i, f in enumerate(self.config.ensemble.params):
            assert(os.path.exists(f))
            logger.info(f)

        if "train" in self.run_type:
            logger.info("Training not supported for ensemble model")
            #self.on_train_start()
            #self.training_loop()
            #self.on_train_end()
