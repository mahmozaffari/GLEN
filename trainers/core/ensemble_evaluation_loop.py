...
# Modified from mmf/trainers/core/evaluation_loop.py to support ensemble evaluation
...
class TrainerEnsembleEvaluationLoopMixin(ABC):

    def retrieve_ensemble_logits(self, batch):
        # Retrieve ensemble logits from aggregated ensemble logits, given the question ids in Batch
        B = batch['question_id'].shape[0]
        ensemble_output = {'scores': torch.zeros((B, 3129)),'losses':{}} # 3129 is the number of answers (hardcoded)
        for i,k in enumerate(batch.question_id):
            ensemble_output['scores'][i,:] = torch.reshape(self.ensemble_logits[k.item()], (1, -1))
        ensemble_output['scores'] = ensemble_output['scores'].to(self.device)
        ...
        with torch.no_grad():
            ...
            while reporter.next_dataset(flush_report=True):
                ...
                for batch in dataloader:
                    ...
                    model_output = self.retrieve_ensemble_logits(prepared_batch)
                    #model_output = model_output.to(self.device)

                    report = Report(prepared_batch, model_output)
                    report = report.detach()

                    meter.update_from_report(report,should_update_loss=False)
